...
abstract class LibNervaWallet<T extends CryptonoteCurrency>
  ...
  Future<String> getTxKeyFor({required String txid}) async {
    if (wallet == null) {
      throw Exception("Cannot get tx key in uninitialized libNervaWallet");
    }
    return csNerva.getTxKey(wallet!, txid);
  }

  void _setListener() {
    if (wallet != null && !csNerva.hasListeners(wallet!)) {
      csNerva.addListener(
        wallet!,
        CsWalletListener(
  ...
  Future<void> open() async {
    ...
    _setListener();
    csNerva.startListeners(wallet!);
    csNerva.startAutoSaving(wallet!);
  ...
  Future<void> save() async {
    ...
    await csNerva.save(wallet!);
  }

  Address addressFor({required int index, int account = 0}) {
    final address = csNerva.getAddress(
      wallet!,
  ...
  Future<CWKeyData?> getKeys() async {
    ...
    try {
      return CWKeyData(
        walletId: walletId,
        publicViewKey: csNerva.getPublicViewKey(wallet!),
        privateViewKey: csNerva.getPrivateViewKey(wallet!),
        publicSpendKey: csNerva.getPublicSpendKey(wallet!),
        privateSpendKey: csNerva.getPrivateSpendKey(wallet!),
      );
    } catch (e, s) {
  ...
  hackToCreateNewViewOnlyWalletDataFromNewlyCreatedWalletThisFunctionShouldNotBeCalledUnlessYouKnowWhatYouAreDoing() async {
    ...
    try {
      password = (await secureStorageInterface.read(
        key: _libNervaWalletPasswordKey(walletId),
      ))!;
    } catch (e, s) {
      throw Exception("Password not found $e, $s");
    }
    wallet = await loadWallet(path: path, password: password);
    return (csNerva.getAddress(wallet!), csNerva.getPrivateViewKey(wallet!));
  }
    ...
    if (!(walletExists(path)) && isRestore != true) {
      ...
      try {
        ...
        await info.updateRestoreHeight(
          newRestoreHeight: csNerva.getRefreshFromBlockHeight(wallet),
          isar: mainDB.isar,
        );
        ...
        this.wallet = wallet;
        await updateNode();
        await csNerva.close(wallet, save: true);
        this.wallet = null;
      } catch (e, s) {
  ...
  Future<void> recover({required bool isRescan}) async {
    if (isRescan) {
      await refreshMutex.protect(() async {
        ...
        unawaited(csNerva.rescanBlockchain(wallet!));
        csNerva.startSyncing(wallet!);
        // unawaited(save());
      });
      unawaited(refresh());
    ...
    await refreshMutex.protect(() async {
      ...
      try {
        ...
        try {
          ...
          final newReceivingAddress =
              ...
              Address(
                ...
                derivationIndex: 0,
                derivationPath: null,
                value: csNerva.getAddress(this.wallet!),
                publicKey: [],
                type: AddressType.cryptonote,
                subType: AddressSubType.receiving,
              );
        ...
        // libNervaWallet?.setRecoveringFromSeed(isRecovery: true);
        unawaited(csNerva.rescanBlockchain(wallet!));
        csNerva.startSyncing(wallet!);
  ...
  Future<bool> pingCheck() async {
    if (_canPing) {
      return csNerva.isConnectedToDaemon(wallet!);
    } else {
      return false;
    }
  }
  ...
  Future<void> updateTransactions() async {
    ...
    final allTxids = await csNerva.getAllTxids(wallet!, refresh: true);
    ...
    final transactions = await csNerva.getTxs(
      wallet!,
      txids: txidsToFetch,
      refresh: false,
    );

    final allOutputs = await csNerva.getOutputs(
      wallet!,
      includeSpent: true,
      refresh: true,
    );
    ...
    for (final tx in transactions) {
      ...
      final txn = TransactionV2(
        walletId: walletId,
        blockHash: null, // not exposed via current cs_nerva
        hash: tx.hash,
        txid: tx.hash,
        ...
        inputs: inputs,
        outputs: outputs,
        version: -1, // not exposed via current cs_nerva
        type: type,
  ...
  Future<Amount> get availableBalance async {
    try {
      return Amount(
        rawValue: csNerva.getUnlockedBalance(wallet!)!,
        fractionDigits: cryptoCurrency.fractionDigits,
      );
    } catch (_) {
  ...
  Future<Amount> get totalBalance async {
    try {
      final full = csNerva.getBalance(wallet!);
      if (full != null) {
        return Amount(
          rawValue: full,
          fractionDigits: cryptoCurrency.fractionDigits,
        ...
        final transactions = await csNerva.getAllTxs(wallet!, refresh: true);
  ...
  Future<void> exit() async {
    ...
    if (wallet != null) {
      csNerva.stopAutoSaving(wallet!);
      csNerva.stopListeners(wallet!);
      csNerva.stopSyncing(wallet!);
      await csNerva.save(wallet!);
    }
  }
  ...
  Future<void> onUTXOsChanged(List<UTXO> utxos) async {
    if (wallet == null) {
      Logging.instance.w(
        "onUTXOsChanged triggered while cs_nerva wallet is null. If this "
        "occurs while not in a nerva wallet this warning can be ignored.",
      );
      return;
    }

    await _utxosUpdateLock.protect(() async {
      final cwUtxos = await csNerva.getOutputs(wallet!, refresh: true);
      ...
      for (final cw in cwUtxos) {
        ...
        if (match.isNotEmpty) {
          ...
          if (u.isBlocked) {
            if (!cw.isFrozen) {
              await csNerva.freezeOutput(wallet!, cw.keyImage);
              // changed = true;
            }
          } else {
            if (cw.isFrozen) {
              await csNerva.thawOutput(wallet!, cw.keyImage);
              // changed = true;
            }
          }
        }
      ...
      // if (changed) {
      //   await libNervaWallet?.updateUTXOs();
      // }
    });
  }
  ...
  Future<bool> updateUTXOs() async {
    ...
    final utxos = wallet == null
        ? <CsOutput>[]
        : await csNerva.getOutputs(wallet!, refresh: true);
  ...
  Future<void> refresh() async {
    ...
    final synced = wallet != null && await csNerva.isSynced(wallet!);
  ...
  Future<void> checkReceivingAddressForTransactions() async {
    ...
    try {
      int highestIndex = -1;
      final entries = await csNerva.getAllTxs(wallet!, refresh: true);
  ...
  Future<FeeObject> get fees async => FeeObject(
    ...
    fast: BigInt.from(csNerva.getTxPriorityHigh()),
    medium: BigInt.from(csNerva.getTxPriorityMedium()),
    slow: BigInt.from(csNerva.getTxPriorityNormal()),
  );
      ...
      if (feeRate is FeeRateType) {
        ...
        switch (feeRate) {
          case FeeRateType.fast:
            feePriority = csNerva.getTxPriorityHigh();
            break;
          case FeeRateType.average:
            feePriority = csNerva.getTxPriorityMedium();
            break;
          case FeeRateType.slow:
            feePriority = csNerva.getTxPriorityNormal();
            break;
          default:
        ...
        try {
          ...
          // cs_nerva may not support this yet properly
          ...
          return await prepareSendMutex.protect(() async {
            ...
            if (outputs.length == 1) {
              pendingTransaction = await csNerva.createTx(
                wallet!,
              ...
              pendingTransaction = await csNerva.createTxMultiDest(
                wallet!,
                outputs: outputs,
                minConfirms: cryptoCurrency.minConfirms,
                currentHeight: height,
  ...
  Future<TxData> confirmSend({required TxData txData}) async {
    try {
      try {
        await csNerva.commitTx(wallet!, txData.pendingTransaction!);
  ...
  Future<int> getRefreshFromBlockHeight() async => wallet == null
      ...
      : csNerva.getRefreshFromBlockHeight(wallet!);

  @override
  int getTxPriorityHigh() => csNerva.getTxPriorityHigh();

  @override
  int getTxPriorityMedium() => csNerva.getTxPriorityMedium();

  @override
  int getTxPriorityNormal() => csNerva.getTxPriorityNormal();
  ...
  Future<void> internalCommitTx(CsPendingTransaction tx) {
    ...
    return csNerva.commitTx(wallet!, tx);
  }
  ...
  Future<BigInt> internalGetUnlockedBalance({int accountIndex = 0}) async {
    ...
    return csNerva.getUnlockedBalance(wallet!, accountIndex: accountIndex)!;
  }
  ...
  void setRefreshFromBlockHeight(int newHeight) {
    ...
    csNerva.setRefreshFromBlockHeight(wallet!, newHeight);
  }
  ...
  Future<void> recoverViewOnly() async {
    await refreshMutex.protect(() async {
      ...
      try {
        ...
        unawaited(csNerva.rescanBlockchain(this.wallet!));
        csNerva.startSyncing(this.wallet!);

        // await save();
        csNerva.startListeners(this.wallet!);
        csNerva.startAutoSaving(this.wallet!);
      } catch (e, s) {
        Logging.instance.e(
...
String _libNervaWalletPasswordKey(String walletName) =>
    "NERVA_WALLET_PASSWORD_${walletName.toUpperCase()}";
...
Future<String> _pathForWalletDir({
  ...
  final walletDire = Directory('${walletsDir.path}/nerva/$name');
